...
class GlobalState:
    ...
    @classmethod
    def get_use_session_connection(cls) -> bool:
        if cls.__use_session_connection is None:
            cls.__use_session_connection = (
                    os.getenv("DBT_DATABRICKS_SESSION_CONNECTION", "False").upper() == "TRUE"
            )
        return cls.__use_session_connection
